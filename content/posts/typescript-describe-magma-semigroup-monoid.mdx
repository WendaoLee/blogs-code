...
同态态射的定义是，对于代数结构 $< M_1,\text{ ⊕ }_1 >$ 与 $< M_2,\text{ ⊕ }_2 >$ ，如果有映射 $h:M_1 \rightarrow M_2$  使得对于 $M_1$ 中的任意元素$x,y$，都能满足 $h(x \text{⊕}_1 y) = h(x)\text{⊕}_2h(y)$，那么则称 h 为代数结构$< M_1,\text{⊕}_1 >$ 与 $< M_2,\text{⊕}_2 >$ 的同态态射，因为 h 维持了相应代数结构的操作。

即：

$$
\text{Homomorphism:=} \forall < M_1,\text{⊕}_1 > ,< M_2,\text{⊕}_2 >, x,y \in M_1  \exists h:M_1 \rightarrow M_2 => h(x \text{⊕}_1 y) = h(x)\text{⊕}_2h(y)
$$
